-- ══════════════════════════════════════════════════════════════════════════════
-- Migration: AI Monthly Report — Scorecard usa nomi puliti da imported_exercises
-- Modifica build_month_scorecard_block() per fare LEFT JOIN con imported_exercises
-- via exercise_slug, usando nome_it (nome italiano standardizzato) quando presente.
-- Fallback su workout_exercises.exercise_name se l'esercizio non è nel catalogo.
--
-- Motivazione: i nomi in workout_exercises sono quelli digitati dal trainer
-- (spesso con case inconsistente, errori, varianti), mentre imported_exercises.nome_it
-- è lo "slug → nome pulito" del catalogo. Il report AI deve mostrare il nome pulito.
-- ══════════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- LEFT JOIN con imported_exercises: se l'esercizio ha uno slug nel catalogo,
    -- usiamo nome_it (nome italiano standardizzato). Altrimenti fallback su
    -- exercise_name (quello digitato dal trainer, anche se "sporco").
...
-- Permessi invariati (già dati nella migration originale, ripetuti per sicurezza)
