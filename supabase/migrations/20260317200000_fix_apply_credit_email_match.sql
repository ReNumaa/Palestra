-- Fix: apply_credit_to_past_bookings — email matching case-insensitive per credits
-- Il lookup sulla tabella credits usava `email = v_email` (case-sensitive),
-- mentre i bookings usavano `lower(email) = v_email`. Se l'email nei credits
-- è salvata con casing diverso, il credito non veniva trovato e la RPC
-- usciva senza applicare nulla.
...
DECLARE
    ...
    v_start_time    TIME;
BEGIN
    -- ── 1. Recupera saldo credits (case-insensitive) ──────────────────────────
