-- ─── Stripe enabled per utente ────────────────────────────────────────────
-- Flag opt-in che abilita il pulsante di ricarica Stripe (+) nella pagina
-- "Le mie prenotazioni" del cliente, a prescindere dal saldo. Quando false
-- (default), la visibilità del pulsante segue la regola standard (saldo ≤ 0).
-- L'admin gestisce il flag dal form "Modifica contatto".

ALTER TABLE profiles ADD COLUMN IF NOT EXISTS stripe_enabled BOOLEAN NOT NULL DEFAULT false;

-- Aggiorna get_all_profiles per restituire anche stripe_enabled
DROP FUNCTION IF EXISTS get_all_profiles();
CREATE OR REPLACE FUNCTION get_all_profiles()
RETURNS TABLE (
    id                      UUID,
    name                    TEXT,
    email                   TEXT,
    whatsapp                TEXT,
    medical_cert_expiry     TEXT,
    medical_cert_history    JSONB,
    insurance_expiry        TEXT,
    insurance_history       JSONB,
    codice_fiscale          TEXT,
    indirizzo_via           TEXT,
    indirizzo_paese         TEXT,
    indirizzo_cap           TEXT,
    documento_firmato       BOOLEAN,
    geo_enabled             BOOLEAN,
    push_enabled            BOOLEAN,
    privacy_prenotazioni    BOOLEAN,
    stripe_enabled          BOOLEAN
) LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    IF NOT is_admin() THEN
        RAISE EXCEPTION 'Solo admin';
    END IF;
    RETURN QUERY
        SELECT p.id, p.name, p.email, p.whatsapp,
               p.medical_cert_expiry::TEXT, p.medical_cert_history,
               p.insurance_expiry::TEXT, p.insurance_history,
               p.codice_fiscale,
               p.indirizzo_via, p.indirizzo_paese, p.indirizzo_cap,
               p.documento_firmato,
               p.geo_enabled,
               p.push_enabled,
               p.privacy_prenotazioni,
               p.stripe_enabled
        FROM profiles p;
END;
$$;

REVOKE ALL ON FUNCTION get_all_profiles FROM public;
GRANT EXECUTE ON FUNCTION get_all_profiles TO authenticated;
