-- ═══════════════════════════════════════════════════════════════════════════
-- Integrazione _offer_next_in_queue in cancel/admin_delete/fulfill
-- ═══════════════════════════════════════════════════════════════════════════
-- Quando una prenotazione viene annullata/eliminata, lo slot torna ad avere
-- un posto libero: se ci sono richieste pending in coda, la prima viene
-- automaticamente passata a 'offered' e la RPC ritorna i dati per consentire
-- al client di mandare la push notification.
--
-- Base: 20260504000000_shared_slot_reset_on_cancel.sql (versione corrente).
-- Aggiunto: blocco "_offer_next_in_queue" + campo `offered_request` nel JSON.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- Offri il posto liberato al primo in coda (se esiste).
    -- Wrappato in EXCEPTION: se la funzione nuova fallisce, NON deve far
    -- fallire la cancellazione (che è già completata sopra).
    BEGIN
        v_offered := _offer_next_in_queue(v_booking.date, v_booking.time, v_booking.slot_type);
    EXCEPTION WHEN OTHERS THEN
        RAISE WARNING '_offer_next_in_queue failed in cancel_booking_with_refund: %', SQLERRM;
        v_offered := NULL;
    END;

    RETURN jsonb_build_object('success', true, 'offered_request', v_offered);
END;
$$;
...
-- ─── admin_delete_booking_with_refund ────────────────────────────────────────
...
BEGIN
    ...
    -- Memorizza i campi prima della DELETE
    v_b_date := v_booking.date;
    v_b_time := v_booking.time;
    v_b_slot_type := v_booking.slot_type;
    ...
    -- Offri al prossimo in coda (failure-safe)
    BEGIN
        v_offered := _offer_next_in_queue(v_b_date, v_b_time, v_b_slot_type);
    EXCEPTION WHEN OTHERS THEN
        RAISE WARNING '_offer_next_in_queue failed in admin_delete_booking_with_refund: %', SQLERRM;
        v_offered := NULL;
    END;

    RETURN jsonb_build_object('success', true, 'credit_refunded', v_refund_amount, 'offered_request', v_offered);
END;
$$;
...
BEGIN
    ...
    BEGIN
        v_offered := _offer_next_in_queue(v_booking.date, v_booking.time, v_booking.slot_type);
    EXCEPTION WHEN OTHERS THEN
        RAISE WARNING '_offer_next_in_queue failed in fulfill_pending_cancellation: %', SQLERRM;
        v_offered := NULL;
    END;
